...
function sel(i){aidx=i;render();var im=imgs[aidx],vw=vp.clientWidth-20,vh=vp.clientHeight-20;zoom=Math.max(1,Math.floor(Math.min(vw/(im.w+2),vh/(im.h+2))));pw.value=Math.min(Math.max(im.w*2,80),500);ph.value=Math.min(Math.max(im.h*2,80),500);draw();drawPv();}
